...
export default function UXProblemView({ description, userJourney }: Props) {
  return (
    <div
      className="custom-scrollbar h-full overflow-auto px-5 py-5 flex flex-col gap-5"
      dir="rtl"
    >
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          {userJourney.map((step, i) => {
            const isProblematic = i === 2; // step 3 is where users drop off
            return (
                ...
                >
                  <p
                    className={`text-[13px] leading-relaxed ${
                      isProblematic ? "text-red-300" : "text-text-muted"
                    }`}
                  >
                    {step}
                  </p>
